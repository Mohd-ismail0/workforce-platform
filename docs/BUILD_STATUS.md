...
- Agent templates (migration 016): a versioned, DECLARATIVE starting point naming a harness and the scope an agent from it may use. **Only a published template can be adopted**, customization may narrow that scope but never widen it (a partly-out-of-scope request is refused whole, not trimmed), and agents **pin the template version** so a newer version does not silently change configuration already in flight. Publishing validates that the named harness is runnable. `GET/POST /templates`, `POST /templates/{id}/publish`, `POST /agents/from-template`; the Agents page publishes, adopts and shows each agent's pinned version.
- Second materially-different harness and a conformance suite: `testdata/harness_b.py` nests the protocol two layers deep through the documented terminal key, asks a `selection` gate over an enum, has NO native continuation, and spawns a grandchild. Five tests prove both doubles satisfy the same contract (same operation shape and business-fact key, without leaking the run id), that a different question shape survives the gate path, that a fresh invocation succeeds with the answer supplied, and that **a killed run takes its whole process tree with it** (the grandchild's pid must be gone after a timeout) — the containment property a harness that never forks cannot show. Both are test doubles, so this proves the plugin contract, not a vendor's executable.
- Per-operation connector certification: the contract gains `Maturity` per action — `unsupported` / `read_only` / `prepare_preview` / `governed_apply` / `verified_apply` — each requiring evidence text. An uncertified action reads as `unsupported` (the safe default, not "assume capable"), and `AllMaturities` lists offered-but-uncertified actions so a gap is visible rather than absent. Tests enforce that every offered action is certified, that a level states evidence, and that **a simulator may not claim `governed_apply` or `verified_apply`** — it has neither reached a provider nor read anything back. The API catalogue exposes the levels and a test locks it; every operation heading in a review now shows its level ("preview only", "applies on approval", "read only", "not supported") with the evidence as the tooltip, because a preview that looks identical whether or not it applies for real is the most misleading thing that screen could do.
...
- Simulator schemas/test data only. Live providers, external network and real write credentials remain disabled. The first real read-only provider was attempted and is **blocked on scope**: the Zoho credentials available on this host (`ZOHO_CLIENT_ID`/`ZOHO_CLIENT_SECRET`/`ZOHO_REFRESH_TOKEN`, used by the `zoho_books` MCP server) return `INVALID_OAUTHSCOPE` (401) from every Zoho **Mail** endpoint, because that token is Zoho Books-scoped. Reading mail read-only requires a Zoho OAuth grant including a Mail read scope and a refresh token minted with it. (Separately, `auth.xsama.org` sits behind Cloudflare and rejects non-browser client signatures with error 1010 — a browser-like User-Agent is required, which is worth knowing before concluding a credential is broken.)
...
- **The second approver exists as a principal, but has not signed in.** `kubra` (name "Kubra Noor", role `approver`, active) was onboarded against the verified Logto subject `te0ro9utw6bu` — the subject was read back from the Logto Management API rather than assumed, because a wrong subject links fine and then simply never authenticates. The requester/approver separation is therefore *exercisable* but still not demonstrated by two real people signing in; that needs Kubra to complete a login.
- Onboarding a second principal was **impossible until this was fixed**: `BootstrapIdentity` reused one existence flag for two checks, so an existing organization left the flag true and the principal insert was skipped. The first administrator worked only because the organization was absent. See the regression test.
