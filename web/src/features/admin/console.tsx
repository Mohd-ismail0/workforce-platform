 ...
 * Technical console.
 *
 * These screens expose the kernel's own vocabulary — gate schemas, run lifecycles, registry
 * transitions, raw payloads. They are kept because they are real working behaviour and are the
 * fastest way to see what the platform is actually doing, but they are NOT the employee
 * experience: an employee should never have to read a business key or a lifecycle state.
 *
 * They now live behind the Admin area, and each will be replaced by a product surface
 * (a decision card, a task workspace, a preview) as those are built. Nothing here is deleted
 * merely because it looks technical; it is moved out of the way of the main flow.
 */
const statuses = [
  "draft",
  "ready",
...
export function Overview({ me, setError }: { me: Me; setError: (v: string) => void }) {
...
export function Projects({ setError }: { setError: (v: string) => void }) {
...
export function Integrations({ setError }: { setError: (v: string) => void }) {
  const [items, setItems] = useState<any[]>([]);
...
export function Agents({ setError }: { setError: (v: string) => void }) {
  const [items, setItems] = useState<any[]>([]);
...
export function Activity({ setError }: { setError: (v: string) => void }) {
